<div class="cnvrstnmssgs index">
	<h2><?php echo __('Cnvrstnmssgs'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('cnvrstn_id'); ?></th>
			<th><?php echo $this->Paginator->sort('profile_id'); ?></th>
			<th><?php echo $this->Paginator->sort('message'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($cnvrstnmssgs as $cnvrstnmssg): ?>
	<tr>
		<td><?php echo h($cnvrstnmssg['Cnvrstnmssg']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($cnvrstnmssg['Cnvrstn']['title'], array('controller' => 'cnvrstns', 'action' => 'view', $cnvrstnmssg['Cnvrstn']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($cnvrstnmssg['Cnvrstnusr']['id'], array('controller' => 'cnvrstnusrs', 'action' => 'view', $cnvrstnmssg['Cnvrstnusr']['id'])); ?>
		</td>
		<td><?php echo h($cnvrstnmssg['Cnvrstnmssg']['message']); ?>&nbsp;</td>
		<td><?php echo h($cnvrstnmssg['Cnvrstnmssg']['created']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $cnvrstnmssg['Cnvrstnmssg']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $cnvrstnmssg['Cnvrstnmssg']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $cnvrstnmssg['Cnvrstnmssg']['id']), null, __('Are you sure you want to delete # %s?', $cnvrstnmssg['Cnvrstnmssg']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Cnvrstnmssg'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Cnvrstns'), array('controller' => 'cnvrstns', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Cnvrstn'), array('controller' => 'cnvrstns', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Cnvrstnusrs'), array('controller' => 'cnvrstnusrs', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Cnvrstnusr'), array('controller' => 'cnvrstnusrs', 'action' => 'add')); ?> </li>
	</ul>
</div>
